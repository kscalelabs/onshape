...
def calculate_plane_equations(points: NDArray, faces: NDArray) -> NDArray:
    ...
    Args:
        points: The points in the mesh.
        faces: The faces in the mesh.

    Returns:
        The plane equations for each face.
    """
...
def calculate_q_matrices(points: NDArray, faces: NDArray, plane_equ_para: NDArray) -> list[NDArray]:
    ...
    Args:
        points: The points in the mesh.
        faces: The faces in the mesh.
        plane_equ_para: The plane equations for each face.
...
class ObjModel:
    def __init__(self, filepath: str) -> None:
        ...
        self.points, self.faces, self.edges = load_obj_file(filepath)
        self.number_of_points = self.points.shape[0]
        self.number_of_faces = self.faces.shape[0]
        self.plane_equ_para = calculate_plane_equations(self.points, self.faces)
        self.q_matrices = calculate_q_matrices(self.points, self.faces, self.plane_equ_para)


class MeshSimplify(ObjModel):
    def __init__(self, input_filepath: str, threshold: float, simplify_ratio: float) -> None:
    ...
    def generate_valid_pairs(self) -> None:
        self.dist_pairs = []
    ...
    def output(self, output_filepath: str) -> None:
